(function () {
  ...
  function xpMultiplier() {
    ...
    return timedMultiplier * permanentMultiplier("xpMultiplier") * loveBonusMultiplier() * careBonusMultiplier();
  }
  ...
  function emberMultiplier() {
    ...
    return levelMultiplier * boostMultiplier * permanentMultiplier("emberMultiplier") * cosmeticEmberMultiplier() * loveBonusMultiplier() * careBonusMultiplier();
  }
  ...
  function upgradedCareForLlamita(action) {
    ...
    if (action === "sleep") {
      ...
      if (startedSleeping) recordDailyCareAction("sleep");
  ...
  function activeCosmeticItem() {
    return itemById(llamitaState.activeCosmetic);
  }

  function cosmeticEmberMultiplier() {
    return Number(activeCosmeticItem()?.cosmeticBonus?.emberMultiplier) || 1;
  }
  ...
  function careBonusMultiplier() {
    return careBonusForState(llamitaState);
  }
  ...
  function upgradedBuyLlamitaItem(itemId) {
    ...
    const purchaseMessage = item.type === "permanent"
      ...
      : item.type === "cosmetic"
        ? `${item.name} is equipped. Your +3% Ember accessory bonus is active.`
  ...
  function useInventoryItem(itemId) {
    ...
    else if (item.category === "clean") baseShowVisual("shower", 2300);
  ...
  function equipCosmetic(itemId) {
    ...
    upgradedSaveLlamitaState();
    upgradedRenderLlamita();
    const activeItem = activeCosmeticItem();
    if (activeItem) {
      showToast(`${activeItem.name} equipped. +3% Ember is active.`);
      setLlamitaMessage("My new accessory is dancing! We earn a tiny bit more Ember together.", 7000);
      baseCreateBurst([activeItem.icon, "\u2726", "\u2665"], "#ffd765", 9);
    } else {
      showToast("Accessory removed. The Ember bonus is paused.");
    }
  }
  ...
  function renderStatsAndGrowth() {
    ...
    if (ui.petLevel) ui.petLevel.textContent = String(llamitaState.level);
    if (ui.petLevelNote) ui.petLevelNote.textContent = nextMilestone
      ? `Next reward at Level ${nextMilestone.level}`
      : "Legendary reward path complete";
    ...
    setEmberAmount(llamitaCareCount, llamitaState.embers);
    setEmberAmount(llamitaShopBalance, llamitaState.embers);
    ...
    if (ui.careBonus) {
      const careMultiplier = careBonusMultiplier();
      ui.careBonus.querySelector("[data-care-bonus-value]").textContent = `x${careMultiplier.toFixed(2)} XP + Ember`;
      const labels = { hunger: "Hungry", joy: "Joy", energy: "Rest", cleanliness: "Clean" };
      ui.careBonus.querySelector("[data-care-bonus-breakdown]").innerHTML = CONFIG.careBonus.statKeys.map((key) => {
        const percent = Math.round((llamitaState[key] / statMaximum(key)) * 100);
        return `<span>${labels[key]} ${Math.max(0, Math.min(100, percent))}%</span>`;
      }).join("");
    }
  }
  ...
  function renderActiveBoosts() {
    ...
    const accessory = activeCosmeticItem();
    const signature = `${llamitaState.activeBoosts.map((boost) => `${boost.id}:${Math.ceil((boost.endsAt - Date.now()) / 1000)}`).join("|")}::${permanent.map((item) => item.id).join("|")}::${accessory?.id || ""}`;
    ...
    `).join("");
    const accessoryMarkup = accessory ? `
      <div class="llamita-boost-chip is-accessory">
        <span>${accessory.icon} ${escapeHtml(accessory.name)}</span>
        <span>+3% Ember</span>
      </div>
    ` : "";
    if (ui.petBoosts) {
      ui.petBoosts.innerHTML = `${permanentMarkup ? '<span class="llamita-effect-heading">Permanent Upgrades</span>' : ""}${permanentMarkup}${accessoryMarkup ? '<span class="llamita-effect-heading">Active Accessory</span>' : ""}${accessoryMarkup}${timedMarkup ? '<span class="llamita-effect-heading">Active Boosts</span>' : ""}${timedMarkup}`;
    }
  ...
  function renderShopStats() {
    ...
    const values = {
      ...
      clean: [llamitaState.cleanliness, statMaximum("cleanliness")]
    };
    Object.entries(values).forEach(([key, pair]) => {
      const element = ui.shopStats.querySelector(`[data-shop-stat="${key}"]`);
      if (!element) return;
      element.textContent = `${Math.round(pair[0])}/${Math.round(pair[1])}`;
    });
  }

  function visibleShopItems(category) {
    const items = CONFIG.shopItems.filter((item) => item.category === category);
    if (category !== "permanent") return items;
    const families = Array.from(new Set(items.map((item) => item.upgradeFamily)));
    return families.map((family) => {
      const tiers = items
        .filter((item) => item.upgradeFamily === family)
        .sort((left, right) => Number(left.tier) - Number(right.tier));
      const active = itemById(llamitaState.permanentUpgrades[family]);
      if (!active) return tiers[0];
      return tiers.find((item) => Number(item.tier) > Number(active.tier)) || active;
    }).filter(Boolean);
  }
  ...
  function renderShop() {
    ...
    setEmberAmount(ui.shopBalance, llamitaState.embers);
    ...
    const items = visibleShopItems(runtime.shopCategory);
    ui.shopGrid.innerHTML = items.map((item) => {
      ...
      const priceMarkup = ownedCosmetic
        ? "Owned"
        : maxed
          ? "Full"
      ...
      const permanentState = item.type !== "permanent"
        ...
        : activePermanent?.id === item.id
          ...
          : activePermanent
            ? `Next upgrade · Replaces ${activePermanent.name}`
            : "First permanent tier";
      return `
        <button class="glow-shop-card" type="button" data-buy-item="${item.id}" data-item-type="${item.type}" ${disabled ? "disabled" : ""}>
          ...
          <span class="glow-shop-price"><span>${priceMarkup}</span>${locked ? `<span class="glow-shop-level">Level ${item.unlockLevel}</span>` : ""}</span>
        </button>
      `;
    }).join("");
  }
  ...
  function renderInventory() {
    ...
    ui.cosmeticsList.innerHTML = llamitaState.cosmetics.length
      ? llamitaState.cosmetics.map((itemId) => {
          ...
          return `
            <div class="inventory-row is-accessory ${active ? "is-active" : ""}">
              <span class="inventory-icon" aria-hidden="true">${item.icon}</span>
              <span class="inventory-copy"><strong>${escapeHtml(item.name)}</strong><span>Animated accessory \u2022 +3% Ember while equipped</span></span>
              <button class="llamita-secondary-button" type="button" data-equip-cosmetic="${item.id}">${active ? "Remove" : "Equip"}</button>
            </div>
          `;
        }).join("")
      : '<div class="llamita-empty-state">Accessories you unlock in the shop will appear here.</div>';
    const hasLegacyKeepsakes = llamitaState.ownedItems.length > 0;
    if (ui.legacySection) ui.legacySection.hidden = !hasLegacyKeepsakes;
    ui.legacyList.innerHTML = hasLegacyKeepsakes
  ...
  function dailyCareMissionById(missionId) {
    return CONFIG.dailyCareMissions.missions.find((mission) => mission.id === missionId) || null;
  }

  function ensureDailyCareMissions() {
    const today = localDateKey();
    if (llamitaState.dailyCare?.date === today && Array.isArray(llamitaState.dailyCare.missions)) return false;
    llamitaState.dailyCare = normalizedDailyCareState(null, today);
    runtime.lastSignatures.daily = "";
    return true;
  }

  function recordDailyCareAction(action) {
    ensureDailyCareMissions();
    let missionCompleted = false;
    llamitaState.dailyCare.missions.forEach((mission) => {
      const definition = dailyCareMissionById(mission.id);
      if (!definition || !definition.actions.includes(action) || mission.claimed || mission.progress >= definition.target) return;
      mission.progress = Math.min(definition.target, mission.progress + 1);
      if (mission.progress >= definition.target) missionCompleted = true;
    });
    if (missionCompleted) showToast("Daily Care Mission complete! Your little reward is ready.");
    runtime.lastSignatures.daily = "";
  }

  function renderDailyCareMissions() {
    if (!ui.careMissionList) return;
    ensureDailyCareMissions();
    ui.careMissionList.innerHTML = llamitaState.dailyCare.missions.map((mission) => {
      const definition = dailyCareMissionById(mission.id);
      if (!definition) return "";
      const complete = mission.progress >= definition.target;
      const status = mission.claimed ? "Claimed" : complete ? "Ready" : `${mission.progress}/${definition.target}`;
      return `
        <article class="daily-care-mission ${complete ? "is-complete" : ""} ${mission.claimed ? "is-claimed" : ""}">
          <span class="daily-care-mission-icon" aria-hidden="true">${definition.icon}</span>
          <span class="daily-care-mission-copy">
            <span class="daily-care-mission-difficulty is-${definition.difficulty.toLowerCase()}">${escapeHtml(definition.difficulty)}</span>
            <strong>${escapeHtml(definition.title)}</strong>
            <span>${escapeHtml(definition.description)}</span>
            <small>${emberAmountMarkup(definition.emberReward)} + ${definition.xpReward} XP</small>
          </span>
          ${complete && !mission.claimed
            ? `<button class="llamita-small-button" type="button" data-claim-care-mission="${definition.id}">Claim</button>`
            : `<span class="daily-care-mission-status">${status}</span>`}
        </article>
      `;
    }).join("");
  }

  function claimDailyCareMission(missionId) {
    ensureDailyCareMissions();
    const mission = llamitaState.dailyCare.missions.find((entry) => entry.id === missionId);
    const definition = dailyCareMissionById(missionId);
    if (!mission || !definition || mission.claimed || mission.progress < definition.target) return;
    mission.claimed = true;
    const emberReward = awardEmbers(definition.emberReward);
    const xpReward = definition.xpReward * xpMultiplier();
    llamitaState.embers += emberReward;
    upgradedGrantLlamitaProgress(definition.xpReward, 0);
    upgradedSaveLlamitaState();
    runtime.lastSignatures.daily = "";
    upgradedRenderLlamita();
    showToast(`Daily Care Mission claimed! +${emberReward} Ember and +${xpReward.toFixed(1)} XP.`);
  }
  ...
  function renderDaily() {
    ...
    ensureDailyCareMissions();
    const today = localDateKey();
    const signature = `${llamitaState.daily.stamps}:${llamitaState.daily.lastClaimDate}:${llamitaState.daily.weeklyReady}:${today}:${llamitaState.embers}:${JSON.stringify(llamitaState.dailyCare)}`;
  ...
  function renderArcadeScores() {
    ...
    if (quizBalance) setEmberAmount(quizBalance, llamitaState.embers);
  }

  function quizProgressionForLevel(level = llamitaState.level) {
    const progression = CONFIG.arcade.quiz.progression;
    const tier = progression.tiers.find((entry) => entry.maxLevel === null || Number(level) <= Number(entry.maxLevel))
      || progression.tiers[progression.tiers.length - 1];
    const dailyNetCap = tier.dailyNetCap === null ? Infinity : Math.max(0, Number(tier.dailyNetCap) || 0);
    const rewardBasis = Number.isFinite(dailyNetCap) ? dailyNetCap : progression.unlimitedRewardBasis;
    return { ...tier, dailyNetCap, rewardBasis };
  }

  function quizMaximumBet(difficulty) {
    const settings = CONFIG.arcade.quiz.difficulties[difficulty];
    const basis = quizProgressionForLevel().rewardBasis;
    return Math.max(CONFIG.arcade.quiz.minimumBet, Math.round((basis * settings.betShare) / 10) * 10);
  }

  function scaledQuizBonus(baseReward, difficulty) {
    const progression = CONFIG.arcade.quiz.progression;
    const levelScale = quizProgressionForLevel().rewardBasis / progression.baseRewardBasis;
    const difficultyScale = CONFIG.arcade.quiz.difficulties[difficulty]?.bonusScale || 1;
    return Math.max(0, Math.round(Number(baseReward || 0) * levelScale * difficultyScale));
  }

  function quizBetSelection() {
    const difficulty = ui.quizDifficulty?.value || "Easy";
    const settings = { ...CONFIG.arcade.quiz.difficulties[difficulty], maxBet: quizMaximumBet(difficulty) };
    const dailyNetCap = quizProgressionForLevel().dailyNetCap;
    ...
    if (Number.isFinite(dailyNetCap) && llamitaState.arcade.quizDaily.bonusEmbers >= dailyNetCap) {
      return { valid: false, bet: 0, difficulty, settings, message: "Daily Quiz Ember limit reached. Choose Play without bet." };
    }
    ...
    if (bet > settings.maxBet) return { valid: false, bet, difficulty, settings, message: `Maximum ${difficulty} bet: ${settings.maxBet} Ember` };
    return { valid: true, bet, difficulty, settings, message: `${difficulty} bet limit: ${settings.maxBet} Ember` };
  }

  function resetQuizDailyAttempts() {
    const today = localDateKey();
    if (llamitaState.arcade.quizDaily.date === today) return false;
    llamitaState.arcade.quizDaily = { date: today, attempts: [], bonusEmbers: 0 };
    upgradedSaveLlamitaState();
    return true;
  }

  function quizDailyKey(category, difficulty) {
    return `${category}::${difficulty}`;
  }

  function quizAttemptUsed(category, difficulty) {
    resetQuizDailyAttempts();
    return llamitaState.arcade.quizDaily.attempts.includes(quizDailyKey(category, difficulty));
  }
  ...
  function updateQuizBetUi() {
    ...
    const category = ui.quizCategory?.value || Object.keys(CONFIG.quizQuestions)[0];
    const usedToday = quizAttemptUsed(category, selection.difficulty);
    ui.quizBetInput.disabled = noBet;
    ui.quizBetInput.max = String(Math.min(llamitaState.embers, selection.settings.maxBet));
    if (noBet) ui.quizBet.textContent = "No bet";
    else setEmberAmount(ui.quizBet, Math.max(0, Number(ui.quizBetInput.value) || 0));
    ...
    if (ui.quizDailyStatus) {
      const progression = quizProgressionForLevel();
      const limitText = Number.isFinite(progression.dailyNetCap) ? String(progression.dailyNetCap) : "Unlimited";
      ui.quizDailyStatus.textContent = usedToday
        ? `Completed today. Level ${llamitaState.level} daily net: ${llamitaState.arcade.quizDaily.bonusEmbers}/${limitText} Ember. ${selection.difficulty} maximum bet: ${selection.settings.maxBet} Ember.`
        : `Available today. Level ${llamitaState.level} daily net: ${llamitaState.arcade.quizDaily.bonusEmbers}/${limitText} Ember. ${selection.difficulty} maximum bet: ${selection.settings.maxBet} Ember.`;
      ui.quizDailyStatus.classList.toggle("is-complete", usedToday);
    }
    if (ui.quizStart) ui.quizStart.disabled = usedToday || !selection.valid;
  }
  ...
  function updateShopCategoryArrows() {
    if (!ui.shopCategories || !ui.shopCategoryArrows) return;
    const maxScroll = Math.max(0, ui.shopCategories.scrollWidth - ui.shopCategories.clientWidth);
    ui.shopCategoryArrows.forEach((button) => {
      button.disabled = button.dataset.shopCategoryScroll === "left"
        ? ui.shopCategories.scrollLeft <= 8
        : ui.shopCategories.scrollLeft >= maxScroll - 2;
    });
  }

  function scrollShopCategories(direction) {
    if (!ui.shopCategories) return;
    const amount = Math.max(140, ui.shopCategories.clientWidth * 0.72) * (direction === "left" ? -1 : 1);
    ui.shopCategories.scrollBy({ left: amount, behavior: prefersReducedMotion ? "auto" : "smooth" });
  }
    ...
    return `
      ...
      <div class="arcade-panel-shell">
        <section class="arcade-game" data-arcade-game="whack">
          ...
          <div class="arcade-bonus-hint">Bonus: hit at least ${CONFIG.arcade.whack.cleanRoundMinimumHits} symbols without touching water or a skull.</div>
        ...
        <section class="arcade-game" data-arcade-game="quiz" hidden>
          ...
          <div class="arcade-scoreline"><span>Best: <strong data-high-score="quiz">0</strong> / 5</span><span>Balance: <strong data-quiz-balance>${emberAmountMarkup(0)}</strong></span></div>
          <div class="quiz-setup" data-quiz-setup>
            ...
            <div class="quiz-bet-note"><span>Your bet</span><strong data-quiz-bet>${emberAmountMarkup(CONFIG.arcade.quiz.defaultBet)}</strong><span data-quiz-multiplier>Easy reward x1.5</span></div>
            <div class="arcade-bonus-hint">Perfect Quiz Bonus: answer every question correctly.</div>
            <div class="quiz-daily-status" data-quiz-daily-status aria-live="polite">Available today: one attempt for this category and difficulty.</div>
  ...
  function buildUpgradeUi() {
    ...
    if (label) label.textContent = "Hungry";
    ...
    const needs = petPanel.querySelector(".llamita-needs");
    if (needs) needs.insertAdjacentHTML("afterend", `
      <div class="llamita-care-bonus" id="llamitaCareBonus" aria-label="Care reward bonus">
        <span class="llamita-care-bonus-label">Care Reward</span>
        <strong data-care-bonus-value>x1.00 XP + Ember</strong>
        <span class="llamita-care-bonus-breakdown" data-care-bonus-breakdown></span>
      </div>
    `);
      ...
      <div class="llamita-subnav-shell">
        ...
        <nav class="llamita-subnav" id="llamitaSubnav" aria-label="Mi llamita areas">
          ${[
            ["pet", "\u2665", "Pet / Home"],
            ["inventory", "\u25c7", "Inventory"],
            ["shop", `<img class="ember-symbol" src="${escapeHtml(CONFIG.currency.iconPath)}" alt="" aria-hidden="true" />`, "Little Glow Shop"],
            ["arcade", "\u2605", "Glow Arcade"],
            ["daily", "\u2600", "Daily Missions & Stamps"],
            ["path", "\u2726", "Reward Path"],
            ["save", "\u2713", "Save / Load"]
    ...
    view.insertAdjacentHTML("beforeend", panelMarkup("shop", "Care and treasures", "Little Glow Shop", "In-game care items, boosts, XP helpers, and animated accessories.", `
      <div class="shop-sticky-summary" id="shopCurrentStats">
        <span class="shop-stats-title">Current Stats</span>
        <span class="shop-stat"><small>Hungry</small><strong data-shop-stat="hunger">0/100</strong></span>
        ...
        <span class="shop-stat is-clean"><small>Clean</small><strong data-shop-stat="clean">0/100</strong></span>
      </div>
      <div class="llamita-upgrade-balance"><span>Your balance</span><strong id="upgradeShopBalance">${emberAmountMarkup(0)}</strong></div>
    `, "llamita-shop-panel"));
    ...
    llamitaShopBody.innerHTML = `
      <div class="shop-category-scroll-shell">
        <button class="shop-category-arrow" type="button" data-shop-category-scroll="left" aria-label="Scroll shop categories left">\u2039</button>
        ...
        <button class="shop-category-arrow" type="button" data-shop-category-scroll="right" aria-label="Scroll shop categories right">\u203a</button>
      </div>
      <div class="llamita-shop-grid-upgrade" id="llamitaShopGridUpgrade"></div>
    `;

    view.insertAdjacentHTML("beforeend", panelMarkup("inventory", "Your tiny collection", "Inventory", "Use consumables, activate one-time boosts, and choose an animated accessory.", `
      ...
      <section class="inventory-section"><div class="inventory-section-head"><h4>Animated Accessories</h4></div><div class="inventory-list" id="cosmeticsList"></div></section>
      <section class="inventory-section" id="legacyKeepsakesSection" hidden><div class="inventory-section-head"><h4>Original Keepsakes</h4></div><div class="inventory-list" id="legacyKeepsakes"></div></section>
    `));
    ...
    view.insertAdjacentHTML("beforeend", panelMarkup("daily", "A little hello each day", "Daily Missions & Stamps", "Complete three changing care missions and collect one warm stamp each day.", `
      <div class="daily-card">
        ...
        <section class="daily-care-card" aria-labelledby="dailyCareMissionTitle">
          <div class="daily-care-card-head">
            <span class="daily-care-card-spark" aria-hidden="true">\u2726</span>
            <div><span>One easy, one medium, one hard</span><h4 id="dailyCareMissionTitle">Today's Care Missions</h4></div>
          </div>
          <div class="daily-care-mission-list" id="dailyCareMissionList"></div>
          <p>New little missions arrive each calendar day.</p>
        </section>
      </div>
    `));
  ...
  function cacheUi() {
    ...
    ui.careBonus = document.querySelector("#llamitaCareBonus");
    ui.petLevel = document.querySelector("#llamitaCurrentLevel");
    ui.petLevelNote = document.querySelector("#llamitaCurrentLevelNote");
    ...
    ui.shopCategoryArrows = Array.from(document.querySelectorAll("[data-shop-category-scroll]"));
    ...
    ui.legacySection = document.querySelector("#legacyKeepsakesSection");
    ...
    ui.careMissionList = document.querySelector("#dailyCareMissionList");
    ...
    ui.quizCategory = document.querySelector("[data-quiz-category]");
    ...
    ui.quizDailyStatus = document.querySelector("[data-quiz-daily-status]");
    ui.quizStart = document.querySelector("[data-start-quiz]");
  }
  ...
  function bindUpgradeEvents() {
    ...
    ui.shopCategoryArrows.forEach((button) => button.addEventListener("click", () => scrollShopCategories(button.dataset.shopCategoryScroll)));
    ui.shopCategories.addEventListener("scroll", updateShopCategoryArrows, { passive: true });
    ui.shopCategories.addEventListener("wheel", (event) => {
      if (Math.abs(event.deltaY) <= Math.abs(event.deltaX) || ui.shopCategories.scrollWidth <= ui.shopCategories.clientWidth) return;
      event.preventDefault();
      ui.shopCategories.scrollLeft += event.deltaY;
    }, { passive: false });
    window.addEventListener("resize", updateShopCategoryArrows, { passive: true });
    llamitaShopBody.addEventListener("click", (event) => {
      ...
      if (category) {
        ...
        renderShop();
        category.scrollIntoView({ behavior: prefersReducedMotion ? "auto" : "smooth", block: "nearest", inline: "center" });
        window.setTimeout(updateShopCategoryArrows, prefersReducedMotion ? 0 : 320);
        return;
      }
    ...
    ui.careMissionList.addEventListener("click", (event) => {
      const claim = event.target.closest("[data-claim-care-mission]");
      if (claim) claimDailyCareMission(claim.dataset.claimCareMission);
    });
  ...
  function arcadeCareRefusal() {
    upgradedApplyLlamitaTime();
    const threshold = Number(CONFIG.arcade.careRefusal?.thresholdPercent) || 25;
    const chance = Number(CONFIG.arcade.careRefusal?.chance) || 0.55;
    const needs = [
      { key: "hunger", visual: "hungry", message: "Could we have a tiny snack before playing? My glow is too hungry right now." },
      { key: "energy", visual: "unwell", message: "My little flame is too sleepy right now. Could I rest before we play?" },
      { key: "cleanliness", visual: "shower", message: "I would love a sparkly bath before our next game, please." }
    ].map((entry) => ({ ...entry, percent: (llamitaState[entry.key] / statMaximum(entry.key)) * 100 }))
      .filter((entry) => entry.percent < threshold)
      .sort((left, right) => left.percent - right.percent);
    if (!needs.length || Math.random() >= chance) return false;
    const need = needs[0];
    baseShowVisual(need.visual, 2600);
    setLlamitaMessage(need.message, 7500);
    showToast(need.message);
    return true;
  }

  function awardArcadeResult(game, score, baseEmbers, baseXp, highScoreBonus, resultElement, specialBonusText = "") {
    ...
    addStats({ joy: CONFIG.arcade.playJoyReward });
    upgradedSaveLlamitaState();
    runtime.lastSignatures.shop = "";
    runtime.lastSignatures.rewards = "";
    upgradedRenderLlamita();
    const highText = isHighScore ? "New High Score! You broke your record and earned bonus Ember! " : "";
    resultElement.textContent = `${highText}${specialBonusText}Ember earned: ${emberEarned}. XP earned: ${xpEarned.toFixed(1)}. Joy +${CONFIG.arcade.playJoyReward}.`;
  ...
  function startWhack() {
    if (arcadeCareRefusal()) return;
    finishWhack(false);
    runtime.whack.running = true;
    runtime.whack.score = 0;
    runtime.whack.hits = 0;
    runtime.whack.hazardsHit = 0;
  ...
  function hitWhack(index) {
    ...
    if (!target) return;
    runtime.whack.hits += 1;
    if (target.id === "water" || target.id === "skull") runtime.whack.hazardsHit += 1;
  ...
  function finishWhack(giveReward = true) {
    ...
    const cleanRound = runtime.whack.hazardsHit === 0 && runtime.whack.hits >= CONFIG.arcade.whack.cleanRoundMinimumHits;
    const baseEmbers = Math.min(CONFIG.arcade.whack.maxBaseEmber, Math.max(10, 10 + score * 2))
      + (cleanRound ? CONFIG.arcade.whack.cleanRoundEmberBonus : 0);
    const xp = (score >= CONFIG.arcade.whack.strongScore ? CONFIG.arcade.whack.strongXp : Math.max(1, Math.floor(score / 3)))
      + (cleanRound ? CONFIG.arcade.whack.cleanRoundXpBonus : 0);
    const cleanText = cleanRound ? "Perfect danger-free round! No water or skull touched. Bonus earned! " : "";
    awardArcadeResult("whack", score, baseEmbers, xp, CONFIG.arcade.whack.highScoreBonus, document.querySelector("[data-whack-result]"), cleanText);
  ...
  function startQuiz() {
    ...
    const difficulty = ui.quizDifficulty?.value || "Easy";
    if (quizAttemptUsed(category, difficulty)) {
      updateQuizBetUi();
      showToast("This category and difficulty is complete for today.");
      return;
    }
    const selection = quizBetSelection();
    if (!selection.valid) {
    ...
    if (arcadeCareRefusal()) return;
    llamitaState.arcade.quizDaily.attempts.push(quizDailyKey(category, difficulty));
  ...
  function finishQuiz() {
    ...
    const perfectRound = score === CONFIG.arcade.quiz.rounds;
    let payout = 0;
    let message = "Better luck next time.";
    if (session.bet === 0 && score >= 3) {
      const baseReward = Number(CONFIG.arcade.quiz.noBetBaseRewards[score]) || 0;
      payout = awardEmbers(scaledQuizBonus(baseReward, session.difficulty) * session.multiplier);
      message = perfectRound ? "Jackpot!" : "Your bright answer streak earned Ember!";
    } else if (score === 3) {
      payout = session.bet;
      ...
      payout = session.bet + awardEmbers(session.bet * (session.multiplier - 1) + scaledQuizBonus(CONFIG.arcade.quiz.jackpotBonus, session.difficulty));
      message = "Jackpot!";
    }
    const perfectBonus = perfectRound ? awardEmbers(scaledQuizBonus(CONFIG.arcade.quiz.perfectBonus, session.difficulty)) : 0;
    const highBonus = isHighScore && score >= 3 ? awardEmbers(scaledQuizBonus(CONFIG.arcade.quiz.highScoreBonus, session.difficulty)) : 0;
    const rawTotalEmbers = payout + highBonus + perfectBonus;
    const refundableBet = score >= 3 ? session.bet : 0;
    const rawBonusEmbers = Math.max(0, rawTotalEmbers - refundableBet);
    const dailyNetCap = quizProgressionForLevel().dailyNetCap;
    const remainingDailyBonus = Number.isFinite(dailyNetCap)
      ? Math.max(0, dailyNetCap - llamitaState.arcade.quizDaily.bonusEmbers)
      : Infinity;
    const awardedBonusEmbers = Math.min(rawBonusEmbers, remainingDailyBonus);
    const totalEmbers = refundableBet + awardedBonusEmbers;
    llamitaState.arcade.quizDaily.bonusEmbers += awardedBonusEmbers;
    llamitaState.embers += totalEmbers;
    const baseXp = score * 2 + (perfectRound ? 5 + CONFIG.arcade.quiz.perfectXpBonus : 0);
    ...
    const highText = isHighScore ? " New High Score! You broke your record and earned bonus Ember!" : "";
    const perfectText = perfectRound ? " Perfect Quiz Bonus: every answer was correct!" : "";
    const capText = awardedBonusEmbers < rawBonusEmbers ? " The daily Quiz Ember limit was reached." : "";
    document.querySelector("[data-quiz-result]").textContent = `${message}${perfectText}${highText}${capText} Ember earned: ${totalEmbers}. XP earned: ${earnedXp.toFixed(1)}. Joy +${CONFIG.arcade.playJoyReward}.`;
  ...
  function bindArcadeEvents() {
    ...
    ui.quizCategory.addEventListener("change", updateQuizBetUi);
